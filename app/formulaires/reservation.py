...
class Demande(Form):
    ...
    ville_dep = TextField(
        validators=[Required(message = 'Veuillez renseigner la ville de départ.'), Length(min=2)], description = "Ville de départ"
    )
    
    cp_dep = TextField(validators = [Length(max=5)],description = 'Code postal')
   
    adresse_dep = TextField(
        validators = [Required(message = 'Veuillez renseigner une adresse'), Length(min=2)],
        description = 'Adresse'
    )
    
    numero_dep = TextField(
        validators = [Required(message = 'Veuillez renseigner un n° de rue'), Length(min=1)], 
        description = 'Numéro'
    )

    date_debut = TextField(
        validators=[Required(message = 'Veuillez renseigner une date de début')],
        description = 'Date de début'
    )

    ville_arr = TextField(
        validators=[Required(message="Veuillez renseigner la ville d'arrivée."),Length(min=2)],
